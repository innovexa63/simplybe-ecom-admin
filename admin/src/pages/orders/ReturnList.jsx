...
export default function ReturnList() {
  ...
  useEffect(() => {
    api.get('/orders')
      .then(res => {
        const withReturns = res.data.filter(o => o.returnStatus && o.returnStatus !== 'None');
  ...
  const handleUpdate = async (id, returnStatus) => {
    setUpdatingId(id);
    try {
      const res = await api.put(`/orders/${id}/return-status`, { returnStatus });
  ...
  const filtered = filterStatus
    ? orders.filter(o => o.returnStatus === filterStatus)
    : orders;
              ...
              <div key={order._id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                <div className="flex flex-wrap justify-between gap-4 mb-4">
                  ...
                  <div className="text-right">
                    <span className={`inline-block px-3 py-1 rounded-lg text-xs font-bold ${RETURN_STATUS_COLORS[order.returnStatus]}`}>
                      {order.returnStatus}
                    </span>
                    <p className="text-xs text-gray-400 mt-1">
                      Requested: {order.returnRequestedAt ? new Date(order.returnRequestedAt).toLocaleDateString('en-GB') : '-'}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid sm:grid-cols-[1fr_auto] gap-4 items-start">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Photo</h4>
                    {order.returnImage ? (
                      <img
                ...
                {order.returnStatus === 'Requested' && (
                  <div className="flex gap-2 mt-4 pt-4 border-t border-gray-100">
                    <button
                      ...
                      className="px-4 py-2 bg-green-600 text-white text-xs font-bold rounded-lg hover:bg-green-700 disabled:opacity-50 cursor-pointer"
                    >
                      Approve Return
                    </button>
                    <button
                      ...
                      disabled={updatingId === order._id}
                      className="px-4 py-2 bg-white border border-red-200 text-red-600 text-xs font-bold rounded-lg hover:bg-red-50 disabled:opacity-50 cursor-pointer"
                    >
                      Reject
                    </button>
                  </div>
